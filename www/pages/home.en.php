<section>
	<h1>Wildspot Works: Custom Made Lightweight Fursuits and Costumes</h1>
	<div class="uk-column-1-2@l">
		<div>
			<?php if ($this->config->commissions->open) { ?>
				<div class="uk-alert-success uk-text-center" uk-alert>
					<a href="en/prices">Now accepting commissions!</a>
				</div>
			<?php } else { ?>
				<div class="uk-alert-danger uk-text-center" uk-alert>
					Currently closed for commissions.
				</div>
			<?php } ?>
		</div>

		<div>
			<p>Are you looking for a fursuit to suit your fur? &ndash; Okay, enough bad pun, but if you are looking to do a fursuit commission you are at the right place! :D <br />
			By using 3D-Printing I craft unique light Fursuits which are perfect for indoor and outdoor use.<br><br>
			My crafting style is usually semi-realistic but get your own <a href="en/gallery">impressions</a> here.<br><br>
			If you want to <a href="en/prices">order your own</a> manufactured fursuit check out the pricing-tab, or just hang around to get some <a href="en/tips">tips</a> to improve your fursuit performance.
		</div>

		<div
			class="consent-container" 
			data-element-type="iframe"
			data-attr-height="720"
			data-attr-width="430"
			data-attr-src="meow.social"
		></div>
</section>
